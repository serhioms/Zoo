...
public class BooleanFieldHandler extends AbstractTargetField  {
	...
	public void assignFields(Field field, LazyRef<?> result,
		...
		try {
			if (target != null && !target.asText().isEmpty()) {
				field.set(result.get(), target.asBoolean());
			} else if( schemaNode.asText() != null && !schemaNode.asText().isEmpty()) {
				field.set(result.get(), Boolean.parseBoolean(evaluateExpr(schemaNode.asText(), cache, maps, exceptionHandler)));
			}
