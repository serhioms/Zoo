...
public class LongFieldHandler extends AbstractTargetField {
	...
	public void assignFields(Field field, LazyRef<?> result,
		...
		try {
			if (target != null && !target.asText().isEmpty()) {
				field.set(result.get(), target.asLong());
			} else if( schemaNode.asText() != null && !schemaNode.asText().isEmpty()) {
				field.set(result.get(), Long.parseLong(evaluateExpr(schemaNode.asText(), cache, maps, exceptionHandler)));
			}
