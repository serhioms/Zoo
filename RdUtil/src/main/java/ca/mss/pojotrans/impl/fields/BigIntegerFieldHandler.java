...
public class BigIntegerFieldHandler extends AbstractTargetField{
	
	@Override
	public void assignFields(Field field, LazyRef<?> result,JsonNode schemaNode, JsonNode valueNode, PojoTransformer transformer, 
		...
		try {
			if (target != null && !target.asText().isEmpty()) {
				field.set(result.get(), new BigInteger(target.textValue()));
			} else if( schemaNode.asText() != null && !schemaNode.asText().isEmpty()) {
				field.set(result.get(), new BigInteger(evaluateExpr(schemaNode.asText(), cache, maps, exceptionHandler)));
			}
